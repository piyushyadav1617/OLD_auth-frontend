...
export function Sidebar() {
    ...
    const routes = [{ icon: <Instagram />, title: "Instagram", to: "https://instagram.com" }]
            ...
            <ProSidebar className='w-full flex-[15%] min-h-screen bg-black ' collapsed={collapse} >
                ...
                >
                    ...
                    <div className={collapse ? '' : "px-4"} >
                        ...
                        <SidebarMenu to='/dashboard' title='Organizations' icon={<OrgnaizationSvg />} />

                        <p className='mb-1 ml-4 text-xs text-gray-500 mt-12' >OTHER</p>
                        <SidebarMenu to='/dashboard/support' title='Support' icon={<SupportSvg />} />
                        <SidebarMenu to='/dashboard/settings' title='Setting' icon={<SettingSvg />} />
                    </div>
...
const SidebarMenu = ({ icon, title, to }: SidebarMenuType) => {
    return (
        <MenuItem component={<Link href={to} />} icon={icon} className='text-white '  >
            {title}
        </MenuItem>
    )
}